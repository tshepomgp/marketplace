...
@section('title', 'Email Hosting Checkout')
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2">
        <form action="{{ route('customer.email-hosting.process') }}" method="POST" id="checkoutForm">
            @csrf
            ...
            <div class="bg-white rounded-lg shadow-lg p-8 mb-6">
                ...
                <div class="bg-blue-50 border-l-4 border-blue-600 p-4 mb-6">
                    <p class="text-blue-900"><strong>Domain:</strong> <span id="displayDomain" class="font-bold">{{ $domain }}</span></p>
            ...
            <div class="bg-white rounded-lg shadow-lg p-8 mb-6">
                ...
                <div class="bg-gray-50 p-6 rounded-lg mb-4">
                    ...
                    <p class="text-2xl font-bold text-mtn-black">{{ $plan->name }}</p>
                ...
                <div class="grid grid-cols-3 gap-4 mb-6">
                    <div>
                        ...
                        <p class="text-xl font-bold text-mtn-black">{{ $plan->storage_gb }} GB</p>
                    </div>
                    <div>
                        <p class="text-gray-600 text-sm">Default Mailboxes</p>
                        <p class="text-xl font-bold text-mtn-black">{{ $plan->mailboxes_included }}</p>
                    </div>
                    <div>
                        <p class="text-gray-600 text-sm">Price/Mailbox</p>
                        <p class="text-xl font-bold text-mtn-yellow">{{ number_format($plan->price_per_mailbox, 0) }} XAF</p>
                    </div>
                </div>

                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Number of Additional Mailboxes *</label>
                    <input type="number" name="mailbox_count" value="{{ $plan->mailboxes_included }}" min="1" max="100" required
            ...
            <div class="bg-white rounded-lg shadow-lg p-8 mb-6">
                ...
                <div class="mb-4">
                    ...
                    <div class="flex items-center">
                        ...
                        <span class="px-3 py-2 bg-gray-100 border border-gray-300 border-l-0 rounded-r-lg">@<span id="domainDisplay">{{ $domain }}</span></span>
                    </div>
                    <p id="mainEmailPreview" class="text-sm text-gray-600 mt-2">Email: <strong>@<span id="domainDisplay">{{ $domain }}</strong></p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-lg p-8 sticky top-24">
            ...
            <div class="space-y-4 mb-6 pb-6 border-b-2 border-gray-200">
                <div class="flex justify-between">
                    ...
                    <span class="font-semibold">{{ number_format($domainPrice, 0) }} XAF</span>
                </div>
                
                <div class="flex justify-between">
                    <span class="text-gray-700">Email Hosting (1st Month)</span>
                    <span class="font-semibold" id="hostingCost">{{ number_format($hostingPrice, 0) }} XAF</span>
                </div>
            </div>

            <div class="flex justify-between mb-6">
                <span class="text-xl font-bold text-mtn-black">Total</span>
                <span class="text-3xl font-bold text-mtn-yellow" id="totalCost">{{ number_format($totalPrice, 0) }} XAF</span>
            </div>

            <div class="bg-blue-50 p-4 rounded-lg text-sm text-blue-900">
                <p><strong>Plan:</strong> {{ $plan->name }}</p>
                <p><strong>Storage:</strong> {{ $plan->storage_gb }} GB/mailbox</p>
                <p class="mt-2 text-xs">Renewal after 1 year: {{ number_format($domainPrice + $hostingPrice, 0) }} XAF/year</p>
            </div>
        </div>
    </div>
</div>
...
function updateTotal() {
    ...
    const planPrice = {{ $plan->price_per_mailbox }};
