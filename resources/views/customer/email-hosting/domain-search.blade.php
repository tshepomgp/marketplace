...
@section('title', 'Find Your Domain')
...
<div class="bg-blue-50 border-l-4 border-blue-600 p-4 mb-8">
    <p class="text-blue-900"><strong>Selected Plan:</strong> {{ $selectedPlan->name }} - {{ number_format($selectedPlan->price_per_mailbox, 0) }} XAF/month + {{ number_format($selectedPlan->domain_price, 0) }} XAF domain</p>
</div>
@endif
...
<div class="bg-white rounded-lg shadow-lg p-8 mb-8">
    ...
    <form method="POST" action="{{ route('customer.email-hosting.check-availability') }}" id="domainSearchForm">
    @csrf
    <!-- ADD THIS LINE - Pass plan_id through form -->
    <input type="hidden" name="plan_id" value="{{ request()->query('plan_id') }}">
...
<div id="resultsContainer" class="hidden mb-8">  <!-- ← MUST BE HERE -->
    ...
    <div id="availableMessage" class="hidden">  <!-- ← MUST BE HERE -->
        <div class="bg-green-50 border-2 border-green-500 rounded-lg p-6 text-center mb-6">
            ...
            <form id="checkoutForm" method="GET" action="{{ route('customer.email-hosting.checkout') }}">
...
function checkDomain() {
    ...
    // Make API call
    fetch('{{ route("customer.email-hosting.check-availability") }}', {
        method: 'POST',
        headers: {
