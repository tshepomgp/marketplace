...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
    <a href="{{ route('customer.products.licence-ordering') }}" class="bg-white rounded-lg shadow p-6 hover:shadow-lg transition">
     ...
     <a href="{{ route('customer.products.vm-ordering') }}" class="bg-white rounded-lg shadow p-6 hover:shadow-lg transition">
        <div class="flex items-center justify-between mb-4">
    ...
    <a href="{{ route('customer.products.storage-ordering') }}" class="bg-white rounded-lg shadow p-6 hover:shadow-lg transition">
        <div class="flex items-center justify-between mb-4">
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        @else
            <div class="text-center py-8">
                ...
                <a href="{{ route('customer.products.vm-ordering') }}" class="inline-block bg-mtn-yellow text-black px-4 py-2 rounded-lg font-semibold hover:bg-mtn-orange transition">
                    Order Now
                </a>
            </div>
        @endif
    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        @if(count($storageOrders) > 0)
            <div class="space-y-3">
                @foreach($storageOrders as $order)
                    <div class="flex justify-between items-center p-3 bg-gray-50 rounded">
                        <div>
                            <p class="font-semibold text-sm">{{ $order->storage_size }} TB - {{ ucfirst($order->storage_type) }}</p>
                            <p class="text-xs text-gray-500">{{ $order->order_number }}</p>
                            <p class="text-xs text-gray-500">{{ $order->monthly_cost }} XAF</p>
                        </div>
                        <span class="px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">
                            {{ ucfirst($order->status) }}
                        </span>
        ...
        @else
            <div class="text-center py-8">
                ...
                <a href="{{ route('customer.products.storage-ordering') }}" class="inline-block bg-mtn-yellow text-black px-4 py-2 rounded-lg font-semibold hover:bg-mtn-orange transition">
                    Order Now
                </a>
            </div>
        @endif
